...
class Interpreter(NodeVisitor):
    def __init__(self, parser, port, outfile=None):
        NodeVisitor.__init__(self)
        self.gcode = GCodeMaker(port, outfile)
        x = GCodeMaker.serialport
        self.parser = parser
        self.GLOBAL_SCOPE = {}
        self.declaredDict = {}
        self.waypointList = {}
    ...
    def visit_Moveto(self, node):
        ...
        self.gcode.move_lin(float(distance), relative, float(speed))
    ...
    def visit_Rotate(self, node):
        if type(node.value).__name__ == 'Var':
            ...
            _type = type(node.value['distance']).__name__
        relative = False if _type == 'Num' else True
        self.gcode.move_rot(float(distance), relative, float(speed))
